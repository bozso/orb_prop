function t_poz_vel_eci = ecef2eci_full(t_poz_vel_ecef)
    global eod

    [jd, jd_frac] = whole_and_frac(t_poz_vel_ecef(:,1));
    ...
    for iii = 1:row
        ...
        [reci(iii,:), veci(iii,:), aeci] = ecef2eci(t_poz_vel_ecef(iii,2:4)', ...
                                    t_poz_vel_ecef(iii,5:end)', ...
                                    [0 0 0]', ttt, jd(iii) + jd_frac(iii),...
                                    lod(iii), xp(iii), yp(iii), 2, 0.0, 0.0);
    end
    t_poz_vel_eci = [t_poz_vel_ecef(:,1), reci, veci];
end
